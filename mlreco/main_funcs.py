    ...
    if 'trainval' in cfg:
        ...
        if cfg['trainval']['log_dir']:
            ...
            if handlers is not None:
                handlers.csv_logger = misc.CSVData(logname)
...
def log(handlers, tstamp_iteration, #tspent_io, tspent_iteration,
    ...
    if torch.cuda.is_available():
        mem = misc.round_decimals(torch.cuda.max_memory_allocated()/1.e9, 3)
    ...
    # Report (stdout)
    if report_step:
        acc   = misc.round_decimals(np.mean(res.get('accuracy',-1)), 4)
        loss  = misc.round_decimals(np.mean(res.get('loss',    -1)), 4)
        tfrac = misc.round_decimals(t_net/t_iter*100., 2)
        tabs  = misc.round_decimals(t_net, 3)
        epoch = misc.round_decimals(epoch, 2)
