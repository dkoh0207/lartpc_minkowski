...
import numpy as np
import mlreco.utils.misc as utils
import re
...
class trainval(object):
    ...
    def __init__(self, cfg):
        ...
        self._learning_rate = self._trainval_config.get(
            'learning_rate')  # deprecate to move to optimizer args
        ...
        if self._optim_args.get('lr') is not None:
            if self._learning_rate is not None:
                warnings.warn(
                    "Learning rate set in two locations. Using rate in optimizer_args")
        else:
    ...
    def make_input_forward(self, data_blob):
        ...
        for key in data_blob:
            assert(len(data_blob[key]) == num_proc_unit)
            ...
            for gpu in range(num_proc_unit):
                ...
                for key in data_blob:
                    ...
                    if isinstance(target, list):
                        ...
                        data = [scale.cuda() if len(
                            self._gpus) else scale for scale in target]
                    else:
                        data = target.cuda() if len(self._gpus) else target
    ...
    def forward(self, data_iter):
        ...
        res_combined = {}
        data_combined = {}
        num_forward = int(self._batch_size /
                          (self._minibatch_size * max(1, len(self._gpus))))
    ...
    def _forward(self, train_blob, loss_blob):
        ...
        with torch.set_grad_enabled(self._train):
            ...
            for label in loss_acc:
                if len(output_keys) and not label in output_keys:
                    continue
                res[label] = [loss_acc[label].cpu().item() if isinstance(
                    loss_acc[label], torch.Tensor) else loss_acc[label]]
            ...
            for key in result.keys():
                if len(output_keys) and not key in output_keys:
                    continue
                if len(result[key]) == 0:
                    continue
                if isinstance(result[key][0], list):
                    res[key] = [[utils.to_numpy(s) for s in x]
                                for x in result[key]]
                else:
                    res[key] = [utils.to_numpy(s) for s in result[key]]
            return res
    ...
    def initialize(self):
        ...
        self._criterion = criterion(self._model_config).cuda() if len(
            self._gpus) else criterion(self._model_config)
        ...
        self._optimizer = optim_class(
            self._net.parameters(), **self._optim_args)

        # learning rate scheduler
        if self._lr_scheduler is not None:
            scheduler_class = eval(
                'torch.optim.lr_scheduler.' + self._lr_scheduler)
            self._scheduler = scheduler_class(
                self._optimizer, **self._lr_scheduler_args)
        else:
            self._scheduler = None

        self._softmax = torch.nn.Softmax(
            dim=1 if 'sparse' in self._model_name else 0)
        ...
        for module in self._model_config['modules']:
            if 'model_path' in self._model_config['modules'][module] and self._model_config['modules'][module]['model_path'] != '':
                model_paths.append(
                    (module, self._model_config['modules'][module]['model_path']))

        if model_paths:
            for module, model_path in model_paths:
                if not os.path.isfile(model_path):
                    raise ValueError(
                        'File not found: %s for module %s\n' % (model_path, module))
                ...
                with open(model_path, 'rb') as f:
                    ...
                    for name in self._net.state_dict():
                        ...
                        if other_name in checkpoint['state_dict']:
                            checkpoint['state_dict'][name] = checkpoint['state_dict'].pop(
                                other_name)

                    bad_keys = self._net.load_state_dict(
